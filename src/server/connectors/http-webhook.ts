...
/**
 * Revoke access via webhook: POSTs payload to PROVISION_WEBHOOK_URL.
 */
export async function runHttpWebhookRevoke(ctx: ProvisionContext): Promise<void> {
  const url = process.env.PROVISION_WEBHOOK_URL?.trim();
  if (!url) {
    throw new Error("PROVISION_WEBHOOK_URL is not set");
  }
  ...
  await withRevokeLifecycle(
    ...
    async () => {
      const headers: Record<string, string> = {
        "Content-Type": "application/json",
        "Idempotency-Key": `revoke-${ctx.requestId}`,
      };
      const bearer = process.env.PROVISION_WEBHOOK_BEARER?.trim();
      if (bearer) {
        headers.Authorization = `Bearer ${bearer}`;
      ...
      if (!res.ok) {
        const text = await res.text().catch(() => "");
        throw new Error(
          `Revoke webhook failed: HTTP ${res.status}${text ? ` — ${text.slice(0, 200)}` : ""}`,
        );
      }
    },
  );
